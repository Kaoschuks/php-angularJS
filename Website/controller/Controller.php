...
Abstract Class Controller extends BasicSecurity
{
    ...
    {
        switch(file_exists("{$controller}.php"))
        {
            case true:
            {
    ...
    private function Routing()
    {
        switch(self::$route)
        {
            case "routed":
            {
                ...
                // self::$pageData = PHPWee\Minify::html(self::generateUI());
...
try 
{
    ...
    if(!isset($_SESSION['jwt']))
    {
        ...
        $_SESSION['jwt'] = json_decode(ApiCaller::sendRequest("", $token, "Accounts/Guest", array(), "POST"))->token;
    }
